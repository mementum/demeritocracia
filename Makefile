...
# to build mkdocs
MKDOCS := mkdocs
MKDOCS_SERVE := $(MKDOCS) serve --clean
MKDOCS_BSERVE := $(MKDOCS_SERVE) &
MKDOCS_BUILD := $(MKDOCS) build --clean

GIT := git
GIT_ADD_ALL := $(GIT) add --all
GIT_COMMIT_ALL := $(GIT) commit -a -m "$(shell date)"
GIT_PUSH := $(GIT) push
...
help:
	...
	@echo
	@echo "Git Targets"
	@echo "  git-status (show current status)"
	@echo
